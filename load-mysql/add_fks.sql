-- MySQL foreign key constraints for Join Order Benchmark (JOB) dataset
-- Converted from PostgreSQL foreign keys

ALTER TABLE title ADD FOREIGN KEY (kind_id) REFERENCES kind_type(id);
ALTER TABLE aka_name ADD FOREIGN KEY (id) REFERENCES name(id);
-- Commented out due to data integrity issues (same as PostgreSQL version)
-- ALTER TABLE cast_info ADD FOREIGN KEY (person_id) REFERENCES aka_name(id);
